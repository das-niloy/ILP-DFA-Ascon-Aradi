...
----------------------------- */
int online_phase3(uint32_t ciphertext[4], uint32_t key[8],
                  uint32_t plaintext[4], uint8_t O[4][32][4],
                  int *distinct_used_total, int reg) {

    uint32_t faulty_plaintext[4];
    uint32_t subkey[4] = { 0x03020100, 0x07060504, 0x0b0a0908, 0x0f0e0d0c};
    uint32_t result[4] = {0};
	for(int i = 0; i < 4; i++){
	    result[i] = plaintext[i] ^ subkey[i];
	}
	sbox(&result[0], &result[1], &result[2], &result[3]);
	result[0] = linear(0, result[0]);
	result[1] = linear(0, result[1]);
	result[2] = linear(0, result[2]);
	result[3] = linear(0, result[3]);
...
int main(){
    ...
    for(int i = 0; i < runs; i++){
        ...
        int total_faults =
            online_phase3(ciphertext, key, plaintext, O,
                          &distinct_used, 0);
    ...
    FILE *fp = fopen("results_singlerow_2lastround.txt", "w");
